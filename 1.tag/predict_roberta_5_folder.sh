...
# Loop over 5 folds
for fold in {1..5}; do
    echo "🔥 Predict on Fold $fold..."
    
    # Define paths for this fold
    FOLD_DIR="$DATA_DIR/fold_$fold"
    OUTPUT_DIR="${OUTPUT_BASE}/roberta_fold_${fold}_lr_${LEARNING_RATE}/predictions"

    MODEL="${OUTPUT_BASE}/roberta_fold_${fold}_lr_${LEARNING_RATE}"
